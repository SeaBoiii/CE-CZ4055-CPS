...
def print_key(actual_key, display=True):
    hex_key = ""
    decoded_key = ""

    for i in actual_key:
        hex_key += "{} ".format(hex(int(i)))[2:]
        decoded_key += "{}".format(chr(i))

    if display:
        print(f"Correct Key: \n{hex_key}")
        print(f"Decoded Key: \n{decoded_key}")

    return((hex_key, decoded_key))
...
def getByteKey(args):
    ...
    cycle, data_arr, no_of_possible_values_of_key_byte, no_of_traces, no_of_power_trace, power_model_matrix, actual_power_model_matrix, plot_graph, excludeCtrl, gui = args
    ...
    for key_byte_guess in range(0, no_of_possible_values_of_key_byte):
        k = key_byte_guess + 32 if excludeCtrl else key_byte_guess
    ...
    for key_byte_guess in range(0, no_of_possible_values_of_key_byte):
        ...
        for i in range(0, no_of_power_trace):
            ...
            # Check to make sure corr_value[0] is a np.float64 because it can be NaN in some situations
            if isinstance(corr_value[0], np.floating):
    ...
    best_pos = np.where(best_correlation_values ==
                        correlation_matrix[x1[0]][y1[0]])
    if (plot_graph):
        plot.figure()
    ...
    if not gui:
        ...
        return (key + 32, x_index, best_correlation_values, x1[0], best_pos) if excludeCtrl else (key, x_index, best_correlation_values, x1[0], best_pos)
...
def main(data, no_of_traces, plot_graph):
    ...
    actual_power_model_matrix = np.transpose(
        actual_power_model_matrix[0:no_of_traces])
